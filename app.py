...
def fetch_and_parse_feed():
    ...
    # We audit urlopen for permitted schemes; the feed URL uses the secure HTTPS scheme.
    with urllib.request.urlopen(req, timeout=10) as response:  # nosec B310
        xml_data = response.read()

    # Secure XML parsing with defusedxml (if installed) or standard ET with nosec annotation
    ...
    for entry in root.findall("atom:entry", ATOM_NS):
        ...
        link = (
            link_elem.attrib["href"]
            if link_elem is not None
            else "https://cloud.google.com/bigquery/docs/release-notes"
        )
        ...
        if not h3_matches:
            ...
            for i in range(len(h3_matches)):
                ...
                day_link = (
                    f"https://cloud.google.com/bigquery/docs/release-notes#{anchor}"
                )
...
def get_releases():
    ...
    if (
        ...
        and (current_time - cache["timestamp"]) < cache["expiry"]
    ):
        return jsonify(
            {
                "source": "cache",
    ...
    try:
        ...
        return jsonify(
            {"source": "network", "cached_at": current_time, "updates": updates}
        )
...
if __name__ == "__main__":
    # Flask runs on http://127.0.0.1:5000 by default. Use environment variable for debug mode.
